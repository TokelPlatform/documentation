...
To interact with tokens, you must have set `-pubkey=YOURPUBKEY` on chain launch, or used the `setpubkey YOURPUBKEY` RPC to set your pubkey after the chain has run. An easy way to know whether this is the case, is if you get the listed error.
...
#### Part 4: Make the token creation RPC
For every token you create, you will need a corresponding satoshi of TKL. 
...
The following transaction id will output in your terminal. This is the associated `tokenid` for your newly created token.
<collapse-text hidden title="Response">
